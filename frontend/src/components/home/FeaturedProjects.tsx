...
export default function FeaturedProjects() {
  ...
  const fetchFeaturedProjects = async () => {
    try {
      // fetch from new server API (server will proxy to Firebase later)
      const BACKEND = import.meta.env.DEV ? 'http://127.0.0.1:4000' : '';
      const res = await fetch(`${BACKEND}/api/projects?featured=true&limit=6`);
      if (!res.ok) {
  ...
  type ImageEntry = { order?: number; url?: string };

  const getFirstImage = (images: unknown): string | null => {
    if (!images || !Array.isArray(images) || images.length === 0) return null;
    const arr = images as ImageEntry[];
