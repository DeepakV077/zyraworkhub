...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // submit to server API — server will persist or forward to Firebase when configured
      const BACKEND = import.meta.env.DEV ? 'http://127.0.0.1:4000' : '';
      let res;
      try {
        res = await fetch(`${BACKEND}/api/contact`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      } catch (networkErr: unknown) {
        // Log the underlying network error for debugging while keeping the thrown message user-friendly
  console.error('Network error when submitting contact form:', networkErr);
        throw new Error('Network error: could not reach the server');
      }

      if (!res.ok) {
        // try to parse an error body for more detail
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
              ...
              <p className="text-gray-600 mb-8 leading-relaxed">
                Whether you are interested in our webinars, design services, or partnership opportunities, we are here to help. Fill out the form and we will get back to you within 24 hours.
              </p>
              ...
              <div className="mt-12 p-6 rounded-xl bg-gradient-to-br from-primary-orange/10 to-accent-yellow/10 border border-primary-orange/20">
                ...
                <a
                  href=" https://chat.whatsapp.com/EkBuvSx9jKlIV002Jd9HqL?mode=ems_share_t "
                  target="_blank"
